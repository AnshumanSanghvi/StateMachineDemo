...
public class LeaveApplicationWorkflowScenarios {
    ...
    public static void userInitializesApplication(StateMachine<LeaveAppState, LeaveAppEvent> stateMachine) {
        if (stateMachine.getState().getId().equals(LeaveAppState.INITIAL)) {
            ...
            stateMachine.setStateMachineError(new StateMachineException("Cannot process application, incorrect source state."));
        }
    }

    public static void userSubmitsApplication(StateMachine<LeaveAppState, LeaveAppEvent> stateMachine) {
        userInitializesApplication(stateMachine);
    ...
    private static void sendEvent(StateMachine<LeaveAppState, LeaveAppEvent> stateMachine, LeaveAppEvent event) {
        boolean eventSent = ReactiveHelper.parseResultToBool(stateMachine, event);
        if (!eventSent) {
            log.warn("TestEvent: {} was not sent to the stateMachine with id: {}, and having state: {}", event, stateMachine.getId(),
