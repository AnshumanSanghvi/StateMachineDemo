    ...
    public static StateMachine<String, String> getStateMachine(String stateMachineId) {
        try {
            return switch (stateMachineId) {
                case LEAVE_APP_WF_V1 ->
                        StateMachineBuilder.createStateMachine(LEAVE_APP_WF_V1, null, null, false, 3, 3);
                case LOAN_APP_WF_V1 -> StateMachineBuilder.createStateMachine(LOAN_APP_WF_V1, null, null, false, 3, 3);
                default ->
                        throw new StateMachineException("Could not find a statemachine builder associated with stateMachineId: " + stateMachineId);
            };
        } catch (Exception ex) {
